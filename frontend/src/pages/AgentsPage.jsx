  ...
  return (
    <div className="space-y-6" data-testid="agents-page">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-[#0A192F] flex items-center justify-center"><Sparkles className="w-5 h-5 text-[#3B82F6]" /></div>
        <div>
      ...
      {agentsInfo?.admin_agent && (
        <div className="bg-[#0A192F] rounded-2xl p-5 text-white relative overflow-hidden">
          ...
          <div className="flex items-center gap-3.5 relative z-10">
            <div className="w-11 h-11 rounded-xl bg-white/10 flex items-center justify-center flex-shrink-0"><Bot className="w-6 h-6 text-[#3B82F6]" /></div>
            <div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-1">
          {agentsInfo?.workflow_steps?.map((step, i) => {
            ...
            return (
              <div key={step} className="flex items-center gap-1">
                <button onClick={() => setSelectedAgent(config)} className="flex flex-col items-center min-w-0 p-2 rounded-xl hover:bg-[#F8F9FA] transition-colors cursor-pointer">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="flex flex-col items-center p-2">
              <div className="w-9 h-9 rounded-xl bg-[#0A192F]/10 flex items-center justify-center mb-1"><Lock className="w-4 h-4 text-[#0A192F]" strokeWidth={1.5} /></div>
              <p className="text-[9px] font-bold text-[#0A192F] text-center">Approvazione</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-[#0A192F]/[0.03] border border-[#0A192F]/10 rounded-2xl flex items-start gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#0A192F]/10 flex items-center justify-center flex-shrink-0"><Eye className="w-4 h-4 text-[#0A192F]" strokeWidth={1.5} /></div>
        ...
        <div className="p-4 bg-[#0A192F]/[0.03] border border-[#0A192F]/10 rounded-2xl flex items-start gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#0A192F]/10 flex items-center justify-center flex-shrink-0"><Lock className="w-4 h-4 text-[#0A192F]" strokeWidth={1.5} /></div>
          ...
          {agentsInfo?.agents?.map(agent => {
            ...
            return (
              <button key={agent.type} onClick={() => setSelectedAgent(agent)}
                className={`w-full bg-white rounded-2xl border p-4 text-left transition-all duration-200 ${active ? 'border-[#0A192F] shadow-md ring-1 ring-[#0A192F]' : 'border-[#E2E8F0] hover:border-[#0A192F]/30 hover:shadow-sm'}`}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-[#0A192F] mb-0.5">{agent.branded_name}</p>
          ...
          {selectedAgent ? (
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-6 shadow-[0_4px_20px_rgba(15,23,42,0.04)] sticky top-28" data-testid="agent-detail">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h2 className="text-lg font-bold text-[#0A192F]">{selectedAgent.branded_name}</h2>
              ...
              {isAdmin && selectedAgent.system_prompt && (
                <div>
                  <div className="flex items-center gap-1.5 mb-2"><Eye className="w-3.5 h-3.5 text-[#0A192F]" /><p className="text-[10px] font-semibold text-[#475569] uppercase tracking-wider">Istruzioni di Sistema (Admin)</p></div>
                  <p className="text-[10px] text-[#475569] mb-2">Visibile solo agli amministratori. Nessuna logica nascosta.</p>
                  <ScrollArea className="h-[240px]">
                    <div className="p-3 bg-[#F8F9FA] rounded-xl border border-[#E2E8F0]"><pre className="font-mono text-xs text-[#0F172A] whitespace-pre-wrap leading-relaxed">{selectedAgent.system_prompt}</pre></div>
                  </ScrollArea>
                </div>
              )}
