...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex" data-testid="login-page">
      <div className="hidden lg:flex lg:w-1/2 bg-[#0A192F] text-white p-12 flex-col justify-between relative overflow-hidden">
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-3.5 p-3.5 bg-white/[0.04] rounded-xl border border-white/[0.06]">
              <div className="w-9 h-9 rounded-lg bg-[#3B82F6]/15 flex items-center justify-center flex-shrink-0">
                <item.icon className="w-4.5 h-4.5 text-[#3B82F6]" strokeWidth={1.5} />
              </div>
              <div>
        ...
        <div className="w-full max-w-md animate-fade-in-up">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nome@esempio.it" required className="rounded-xl border-[#E2E8F0] h-12 text-sm focus:border-[#0A192F] focus:ring-1 focus:ring-[#0A192F]" data-testid="login-email-input" />
            </div>

            <div className="space-y-2">
              <Label htmlFor="password" className="text-xs font-semibold text-[#475569] uppercase tracking-wider">Password</Label>
              <div className="relative">
                <Input id="password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="La tua password" required className="rounded-xl border-[#E2E8F0] h-12 text-sm pr-10 focus:border-[#0A192F] focus:ring-1 focus:ring-[#0A192F]" data-testid="login-password-input" />
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-xs font-medium text-[#0A192F] hover:underline" data-testid="forgot-password-link">
                Password dimenticata?
              </Link>
            </div>

            <Button type="submit" disabled={loading} className="w-full bg-[#0A192F] hover:bg-[#0B243B] text-white rounded-xl h-12 text-sm font-semibold shadow-sm transition-all duration-200 hover:shadow-md" data-testid="login-submit-btn">
          ...
          <p className="mt-8 text-center text-sm text-[#475569]">
            Non hai un account?{' '}
            <Link to="/register" className="text-[#0A192F] font-semibold hover:underline" data-testid="register-link">Registrati</Link>
          </p>
        </div>
      </div>
    </div>
